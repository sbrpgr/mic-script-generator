...
- Current static asset cache version is `20260501-09`; bump this version whenever `app.js` or `styles.css` changes.
...
| 녹음 파일 텍스트 변환 | `/tools/audio-file-transcription/` | 녹음 파일 텍스트 변환, 휴대폰 녹음 텍스트 추출, 음성 파일 STT, m4a 텍스트 변환 | L | Browser-only beta. Defaults to a `whisper-base` quality profile, keeps `whisper-tiny` as the fast fallback, and must keep file length limits, non-storage quality warnings, in-progress feedback, and sentence-ending line break option visible. |
