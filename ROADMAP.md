...
- Current static asset cache version is `20260501-05`; bump this version whenever `app.js` or `styles.css` changes.
...
| 녹음 파일 텍스트 변환 | `/tools/audio-file-transcription/` | 녹음 파일 텍스트 변환, 휴대폰 녹음 텍스트 추출, 음성 파일 STT, m4a 텍스트 변환 | L | Browser-only beta. Uses on-demand Transformers.js/Whisper model loading with mixed precision and fp32 fallback; keep file length limits, local-processing warnings, in-progress feedback, and sentence-ending line break option visible. |
