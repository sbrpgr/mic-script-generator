...
- Current static asset cache version is `20260501-14`; bump this version whenever `app.js` or `styles.css` changes.
...
| 녹음 파일 텍스트 변환 | `/tools/audio-file-transcription/` | 녹음 파일 텍스트 변환, 휴대폰 녹음 텍스트 추출, 음성 파일 STT, m4a 텍스트 변환 | L | Browser-only beta. Defaults to the tested `whisper-tiny` stable profile, hides unstable model/device choices from the UI, applies conversation-preserving edge-silence and quiet-speech preprocessing, and must keep file length limits, non-storage quality warnings, in-progress feedback, repetition cleanup, and sentence-ending line break option visible. |
